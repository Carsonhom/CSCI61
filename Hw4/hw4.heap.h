...
class Heap {
 public:
  ...
  int Pop();
  void Print();
  // (a) [30 points] Print the first 4 rows of the heap in tree shape.
  ...
  //    4     9    18
  void PrintTree();
  // (b) [40 points] Returns index of array if n is in heap. Returns -1
  // otherwise.
  int Find(int n);
  // (c) [30 points] Insert n into heap if not already on heap. Otherwise do
  // nothing.
